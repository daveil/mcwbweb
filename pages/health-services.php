
<section id="service-banner" class="full-banner section">
	<img src="img/medium/lolo-banner.jpg" alt="" class="img-responsive" width="100%"/>
	
	
</section>

<section id="services" class="container text-center section">
	<h2>Our focus is on service <br/> of the <b><i>highest quality</i></b>.</h2>
	<br/>
	<div id="open-247">
		<span>OPEN</span>
		<div>
		<span class="counter">24</span>/7
		</div>
		<span>Health Services</span>
	</div>
	<ul>
		<li>Adult and Pediatric ICU</li>
		<li>Ambulance</li>
		<li>Emergency Room</li>
		<li>Delivery Room</li>
		<li>Operating Room</li>
		<li>Laboratory</li>
		<li>Normal & Pathologic Nurses</li>
		<li>Pharmacy</li>
		<li>Radiology Department</li>
		<li>X-ray & CT Scan</li>
	</ul>
</section>
<section id="schedule-banner" class="full-banner section">
	<img src="img/medium/doctor-consultation-banner.jpg" alt="" class="img-responsive"  width="100%"/>
</section>
<section id="schedules" class="container">
	<h3>Schedules</h3>
	<div class="row">
		<div class="col-md-8 col-md-offset-2">
			<table class="table">
				<thead>
					<tr>
						<th>Department</th>
						<th>Weekdays</th>
						<th>Saturday</th>
						<th>Sunday</th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td>OPD Consultation</td>
						<td>8 AM  - 5PM</td>
						<td>8 AM  - 5PM</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>24 Hrs. Holter monitoring</td>
						<td>8 AM  - 4PM</td>
						<td>8 AM  - 4PM</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>2D Echo</td>
						<td>8 AM  - 3PM</td>
						<td>8 AM  - 3PM</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>Stress Test</td>
						<td colspan="3">By Appointment</td>
					</tr>
					<tr>
						<td>EEG Unit</td>
						<td>8 AM  - 4PM</td>
						<td>8 AM  - 4PM</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>Pulmonary Unit</td>
						<td>8 AM  - 4PM</td>
						<td>8 AM  - 4PM</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>Ultrasound</td>
						<td>
							<small>MON</small>
							<div>1PM - 3PM</div>
							<small>TUE</small>
							<div>9AM - 12PM</div>
							<small>WED &mdash; FRI</small>
							<div>11AM - 2PM</div>
							
						</td>
						<td> 11AM &mdash; 2PM</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>Mammography</td>
						<td>8 AM  - 3PM</td>
						<td>8 AM  - 3PM</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>Endoscopy</td>
						<td colspan="3">By Appointment</td>
					</tr>
					<tr>
						<td>Laparoscopy</td>
						<td colspan="3">By Appointment</td>
					</tr>
					<tr>
						<td>Oncology Infusion Unit</td>
						<td>8 AM  - 5PM</td>
						<td>8 AM  - 5PM</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>Dental Services</td>
						<td colspan="3">By Schedule</td>
					</tr>
					<tr>
						<td>Dialysis (NEPHRO)</td>
						<td>8 AM  - Onwards</td>
						<td>8 AM  - Onwards</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>Physical Therapy</td>
						<td >8 AM  - 4PM</td>
						<td >8 AM  - 4PM</td>
						<td>&mdash;</td>
					</tr>
					<tr>
						<td>Occupational Therapy</td>
						<td colspan="3">By Appointment</td>
					</tr>
					<tr>
						<td>Speech Therapy</td>
						<td colspan="3">By Appointment</td>
					</tr>
				</tbody>
			</table>
			<p>To reserve an appointment or check availability.</p>
			<div id="book-wrapper" class="text-center">
				<button class="btn btn-primary">Book Online</button>
			</div>
		</div>
	</div>
</section>
<section id="specialists-banner" class="full-banner section">
	<img src="img/medium/operation-banner.jpg" alt="" class="img-responsive"  width="100%"/>
</section>
<section id="specialists" class="container section">
	<h3>Specialists</h3>
	<div class="contatiner">
	<div class="row"> 
			<div class="col-md-4 specialists-group">
				<h4>O<span>bstetrics</span> G<span>ynecology</span></h4>
				<ul>
					<li class="separator"></li>
				</ul>
			</div>
			<div class="col-md-4 specialists-group">
				<h4>C<span>linical</span> P<span>athology</span></h4>
				<ul>
					<li class="separator"></li>
				</ul>
			</div>
			<div class="col-md-4 specialists-group">
				<h4>R<span>adiology</span></h4>
				<ul>
					<li class="separator"></li>
				</ul>
			</div>
		</div>
		<div class="row">
			<div class="col-md-3">
				<h4>I<span>nternal</span> M<span>edcine</span></h4>
				<ul>
					<li class="separator"></li>
					<li>General Internal Medicine</li>
					<li>Cardiology</li>
					<li>Oncology</li>
					<li>Nephology</li>
					<li>Neurology</li>
					<li>Psychiatry</li>
					<li>Pulmonology</li>
				</ul>
			</div>
			<div class="col-md-3">
				<h4>S<span>urgery</span></h4>
				<ul>
					<li class="separator"></li>
					<li>General Surgery</li>
					<li>Neurosurgery</li>
					<li>Opthalmology</li>
					<li>Otorhinolaryngology</li>
					<li>Urology</li>
					<li>Anesthesiology</li>
				</ul>
			</div>
			<div class="col-md-3">
				<h4>G<span>eneral</span> P<span>ediatrics</span></h4>
				<ul>
					<li class="separator"></li>
					<li>Cardiology</li>
					<li>Neonatology</li>
					<li>Pulmonology</li>
					<li>Developmental Pediatrics</li>
				</ul>
			</div>
			<div class="col-md-3">
				<h4>R<span>ehabilitation</span> M<span>edicine</span></h4>
				<ul>
					<li class="separator"></li>
					<li>Occupational medicine</li>
				</ul>
			</div>
		</div>
		
	</div>
</section>